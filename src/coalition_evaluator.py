            ...
            for candidate in set(voting_data[0, :]):  # Potential strategic votes
                ...
                for member in coalition:
                    if candidate in new_voting_array[:, member]:
                        ...
                        if (
                            idx != 0
                        ):  # Only consider if candidate is not already the top choice
                            new_voting_array[:, member] = np.roll(
                                new_voting_array[:, member], -idx
                            )
                            actual_coalition.append(member)

                actual_coalition.sort()  # Ensure the actual coalition list is ordered for consistency
                scenario_key = (
                    tuple(actual_coalition),
                    candidate,
                )  # Define scenario based on actual changes
                ...
                if scenario_key not in evaluated_scenarios:
                    ...
                    if new_winner != original_winner:
                        ...
                        happiness_diff = {
                            member: new_happiness.happiness_level_dict[
                                f"voter_{member}"
                            ]
                            - original_happiness.happiness_level_dict[f"voter_{member}"]
                            for member in actual_coalition
                        }
                        if all(
                            change > 0 for change in happiness_diff.values()
                        ):  # Check all members are happier
                            evaluated_scenarios.add(
                                scenario_key
                            )  # Mark this scenario as evaluated
                            total_happiness_diff = (
                                new_happiness.total - original_happiness.total
                            )
    ...
    for r in range(1, num_voters + 1):
        for coalition in combinations(range(num_voters), r):
            all_possible_combinations = product(
                *[combinations(candidates, 2) for _ in coalition]
            )
            for new_tops in all_possible_combinations:
                ...
                for voter_idx, voter in enumerate(coalition):
                    ...
                    new_preferences = [
                        cand for cand in new_top if cand in candidates
                    ] + [cand for cand in candidates if cand not in new_top]
                ...
                if new_winner != original_winner:
                    ...
                    for voter in coalition:
                        happiness_change = (
                            temp_happiness.happiness_level_dict[f"voter_{voter}"]
                            - original_happiness.happiness_level_dict[f"voter_{voter}"]
                        )
                    ...
                    if all_happier and is_new_coalition_valid(
                        coalition, successful_coalitions
                    ):
                        total_happiness_diff = (
                            temp_happiness.total - original_happiness.total
                        )
                        successful_coalitions.append(
                            (
                                coalition,
...
def evaluate_coalition_strategic_voting_veto(voting_data: VotingArray):
    ...
    for r in range(1, num_voters + 1):
        for coalition in combinations(range(num_voters), r):
            for new_least_preferred_combination in product(
                candidates, repeat=len(coalition)
            ):
                new_voting_array = voting_data.copy()
                happiness_changes = {}
                strategic_changes = {}
                ...
                for voter_idx, voter in enumerate(coalition):
                    ...
                    if original_least_preferred != new_least:
                        updated_preferences = [
                            c for c in voting_data[:, voter] if c != new_least
                        ] + [new_least]
                ...
                if new_winner != original_winner:
                    ...
                    for voter in coalition:
                        if voter in strategic_changes:
                            happiness_change = (
                                temp_happiness.happiness_level_dict[f"voter_{voter}"]
                                - original_happiness.happiness_level_dict[
                                    f"voter_{voter}"
                                ]
                            )
                            happiness_changes[voter] = happiness_change
                    ...
                    if all_happier and subset_valid:
                        coalition_signature = (
                            tuple(sorted(effective_coalition)),
                            new_winner,
                        )
                        ...
                        successful_coalitions.append(
                            (
                                coalition_signature,
                                {v: strategic_changes[v] for v in effective_coalition},
                                original_winner,
                                new_winner,
                                happiness_changes,
                                total_happiness_diff,
...
def evaluate_coalition_strategic_voting_borda(voting_data: VotingArray):
    ...
    for r in range(1, num_voters + 1):
        for coalition in combinations(range(num_voters), r):
            ...
            for new_prefs_combination in product(
                permutations(candidates), repeat=len(coalition)
            ):
                new_voting_array = voting_data.copy()
                happiness_changes = {}
                strategic_changes = {}
                        ...
                        if coalition_signature not in unique_coalitions:
                            ...
                            successful_coalitions.append(
                                (
                                    coalition_signature,
                                    {
                                        v: strategic_changes[v]
                                        for v in effective_coalition
                                    },
                                    original_winner,
                                    new_winner,
                                    happiness_changes,
...
def remove_redundant_coalitions(successful_coalitions):
    ...
    for current_coalition in successful_coalitions:
        ...
        for other_coalition in successful_coalitions:
            if (
                set(other_coalition[1]).issubset(set(current_coalition[1]))
                and current_coalition != other_coalition
            ):
...
def print_results_coalition_strategic_voting_plurality(successful_coalitions_plurality):
    ...
    for (
        ...
        new_winner,
        happiness_diff,
        overall_happiness_diff,
    ) in successful_coalitions_plurality:
...
def print_results_coalition_strategic_voting_for_two(successful_coalitions):
    ...
    for coalition_data in successful_coalitions:
        ...
        formatted_strategic_votes = ", ".join(
            [f"Voter_{v}: {votes}" for v, votes in strategic_votes.items()]
        )

        # Format the happiness changes for printing
        formatted_happiness_changes = ", ".join(
            [f"Voter_{v}: {change:.3f}" for v, change in happiness_changes.items()]
        )
        ...
        print(
            f"Changes in Happiness Levels for Coalition Members: {formatted_happiness_changes}\n"
        )
        print(f"Overall Happiness Change: {overall_happiness_diff:.3f}\n")
...
def print_results_coalition_strategic_voting_veto(successful_coalitions_veto):
    ...
    for (
        coalition_signature,
        strategic_changes_dict,
        original_winner,
        new_winner,
        happiness_changes,
        overall_happiness_diff,
    ) in successful_coalitions_veto:
        ...
        formatted_changes = ", ".join(
            [f"Voter_{v} = {lp}" for v, lp in strategic_changes_dict.items()]
        )
        formatted_happiness_changes = ", ".join(
            [f"Voter_{v}: {change:.3f}" for v, change in happiness_changes.items()]
        )
...
def print_results_coalition_strategic_voting_borda(successful_coalitions_borda):
    ...
    for (
        ...
        happiness_changes,
        overall_happiness_diff,
    ) in successful_coalitions_borda:
        effective_coalition, _ = coalition_signature
        formatted_members = ", ".join([f"Voter_{v}" for v in effective_coalition])
        formatted_changes = ", ".join(
            [
                f"Voter_{v} changes: {changes}"
                for v, changes in strategic_changes_dict.items()
            ]
        )
        formatted_happiness_changes = ", ".join(
            [f"Voter_{v}: {change:.3f}" for v, change in happiness_changes.items()]
...
def analyze_coalitions(
    n_scenarios, n_voters_list, n_candidates_list, voting_scheme_name, evaluate_function
):
    metrics = {
        "average_coalitions": [],
    ...
    for n_voters in n_voters_list:
        for n_candidates in n_candidates_list:
            ...
            for _ in range(n_scenarios):
                ...
                overall_happiness_change = (
                    sum(data[5] for data in successful_coalitions)
                    / len(successful_coalitions)
            ...
            total_overall_happiness_change.append(
                scenario_happiness_change / n_scenarios
            )
